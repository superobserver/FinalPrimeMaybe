#!/usr/bin/env python3
"""
Mirror Bloom Symmetry — Stricter Einselection for Negative-Plane Blooms
Positive-mirror set is left unchanged (already excellent).
"""
...
# Correct global primes >5 coprime to 90
holes = []
...
print("Correct global primes:", holes[:30], "...")
...
# STRICTER einselection for negative-plane (higher prominence + curvature check)
peaks_neg, _ = find_peaks(D_neg, prominence=0.25, distance=40)   # stricter
negative_zeros = t_frames[peaks_neg]

# Positive-plane mirror (unchanged, already excellent)
peaks_pos, _ = find_peaks(D_pos, prominence=0.08, distance=30)
positive_zeros = t_frames[peaks_pos]

print(f"Negative-plane zeros (stricter filter): {len(negative_zeros)}")
...
ani.save('pnt_correction_comparison_negative_stricter_vs_positive.mp4', writer='ffmpeg', fps=25, dpi=180)
print("✅ Video saved as 'pnt_correction_comparison_negative_stricter_vs_positive.mp4'")

plt.tight_layout()
plt.show()

print("\nError comparison (final x = {:.0f})".format(x[-1]))
print("Negative zeros (stricter filter) error: {:.2f}".format(np.abs(actual_pi[-1] - pnt_neg[-1])))
