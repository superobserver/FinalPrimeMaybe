#!/usr/bin/env python3
"""
Normalized Dial Test: Resonance Skew vs. σ
All curves normalized to their own maximum so alignment at σ = 1/2 is clear.
"""
...
# Normalize each curve to its own max |S| for fair shape comparison
res_norm = {}
for sigma in sigmas:
    max_abs = np.max(np.abs(res[sigma]))
    res_norm[sigma] = res[sigma] / max_abs if max_abs > 0 else res[sigma]
...
for i, sigma in enumerate(sigmas):
    label = f'σ = {sigma} (normalized resonance)'
    ax.plot(t_values, res_norm[sigma], color=colors[i], lw=1.8, label=label)
...
ax.set_title('Normalized Dial Test: Resonance Skew vs. σ\n'
             'Maximal alignment and signal intensity occur exactly at σ = 1/2')
ax.grid(True, alpha=0.3)
ax.legend()
plt.tight_layout()
plt.savefig('dial_test_normalized.png', dpi=300, bbox_inches='tight')
plt.show()