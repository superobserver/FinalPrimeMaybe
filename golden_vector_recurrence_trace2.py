#!/usr/bin/env python3
"""
Corrected Golden Vector Trace — Global True-Prime Hole Collection + MP4 Video
The algebraic ideal is now feeding the clock-face system the correct deterministic holes.
"""
...
def get_holes(n_operators):
    """Global collection: only true primes (correct deterministic holes)."""
    candidates = []
    ...
    for k in classes:
        ...
        for m in range(len(amp)):
            if amp[m] == 0:
                ...
                if p > 5:
                    candidates.append(p)
    # Filter true primes and take the smallest n_operators
    true_primes = sorted(set(p for p in candidates if is_prime(p)))
    return np.array(true_primes[:n_operators])

small_holes = get_holes(N_SMALL)
expanded_holes = get_holes(N_EXPANDED)

print("Small pool (true primes):", small_holes)
print("Expanded pool (true primes):", expanded_holes)
...
# ====================== VIDEO OUTPUT ======================
ani.save('golden_vector_recurrence_trace_corrected.mp4', writer='ffmpeg', fps=35, dpi=240)
print("✅ Video saved as 'golden_vector_recurrence_trace_corrected.mp4'")
