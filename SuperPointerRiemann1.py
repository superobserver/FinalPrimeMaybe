...
# Extend with skew-law predictions for higher k if needed
c, r = 1.14, 4.96
...
# Smooth main term li(x)
def li(x):
    return expi(np.log(x)) - expi(np.log(2)) if x > 2 else 0

x_plot = np.logspace(1, np.log10(MAX_X), 2000)

main_term = np.array([li(xi) for xi in x_plot])

# Actual prime counting function
pi_actual = np.cumsum(np.isin(x_plot, primes))  # step function
# Better: cumulative count
...
for tn in super_pointers:
    ...
    amp = x_plot**0.5 / np.abs(0.5 + 1j*tn)
...
print("Error decreases rapidly with the number of super-pointers used.")